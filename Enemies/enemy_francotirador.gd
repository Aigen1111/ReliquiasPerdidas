# enemy_francotirador.gd — Tirador Lento (ranged)
# FIX: setup() ahora pasa team = "enemy".
extends "res://Enemies/enemy.gd"

@export var preferred_distance: float = 380.0
@export var flee_distance:      float = 160.0
@export var aim_duration:       float = 1.2
@export var shoot_cooldown:     float = 2.8
@export var bullet_damage:      float = 35.0
...
func _behavior(delta: float) -> void:
	...
	match state:
		...
		State.AIM:
			velocity = Vector2.ZERO
			var blink: float = fmod(state_timer, 0.2)
			animated_sprite.modulate = Color(1.8, 0.3, 0.1) if blink < 0.1 else Color.WHITE
			if state_timer <= 0.0:
				_fire()
