# MainMenu.gd
# Menú de inicio: Nueva Partida / Cargar Partida (con selector de slots) / Salir.
extends Control
...
const BTN_DISABLED := Color(0.1, 0.1, 0.12)
...
func _build_ui() -> void:
	...
	# ── Pantalla de selección de slot (oculta al inicio) ──
...
func _populate_load_menu() -> void:
	...
	for meta in SLOT_META:
		...
		_add_button(_load_menu_box, "%s — %s" % [meta["label"], date_str],
